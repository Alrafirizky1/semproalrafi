...
if (empty($_SESSION['admin'])) {
    ...
    $query = mysqli_query($config, "SELECT * FROM tbl_surat_keluar WHERE id_surat_keluar='$id_surat_keluar'");
    if (mysqli_num_rows($query) > 0) {
        while ($row = mysqli_fetch_array($query)) {
                                            ...
                                            <table>
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td width="1%">:</td>
                                                        <td width="86%">' . $row['tanggal_surat'] . '</td>
                                                    </tr>
                                                    <td width="13%">Asal Surat</td>
                                                    <td width="1%">:</td>
                                                    <td width="86%">' . $row['pengirim'] . '</td>
                                                    </tr>
                                                    <tr>
            ...
            if (empty($row['file'])) {
                ...
                $ekstensi = array('jpg', 'png', 'jpeg', 'html');
                $ekstensi2 = array('doc', 'docx', 'pdf');
                $file = $row['file'];
                $x = explode('.', $file);
                $eks = strtolower(end($x));

                if (in_array($eks, $ekstensi) == true) {
                    echo '<img class="gbr" data-caption="' . date('d M Y', strtotime($row['no_surat'])) . '" ' . $row['file'] . '"/>';
                } else {
