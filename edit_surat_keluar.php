...
if (isset($_REQUEST['submit'])) {
    ...
    $id_surat_keluar = $_REQUEST['id_surat_keluar'];
    ...
    if ($file != "") {
        ...
        if (in_array($eks, $ekstensi) == true) {
            if ($ukuran < 2300000) { // Batasi ukuran file max 2.3MB
                ...
                $query = mysqli_query($config, "SELECT file FROM tbl_surat_keluar WHERE id_surat_keluar='$id_surat_keluar'");
                ...
                if (move_uploaded_file($_FILES['file']['tmp_name'], $target_dir . $file)) {
                    ...
                    $query = mysqli_query($config, "UPDATE tbl_surat_keluar SET no_surat='$no_surat', pengirim='$pengirim', tujuan='$tujuan', tanggal_surat='$tanggal_surat', file='$file' WHERE id_surat_keluar='$id_surat_keluar'");
        ...
        $query = mysqli_query($config, "UPDATE tbl_surat_keluar SET no_surat='$no_surat', pengirim='$pengirim', tujuan='$tujuan', tanggal_surat='$tanggal_surat' WHERE id_surat_keluar='$id_surat_keluar'");
    ...
    $id_surat_keluar = mysqli_real_escape_string($config, $_REQUEST['id_surat']);
    $query = mysqli_query($config, "SELECT id_surat_keluar, no_surat, tanggal_surat, pengirim, tujuan FROM tbl_surat_keluar WHERE id_surat_keluar='$id_surat_keluar'");
    list($id_surat_keluar, $no_surat, $tanggal_surat, $pengirim, $tujuan) = mysqli_fetch_array($query);
?>
